...
- animation_options (dict; default {    frame: {        redraw: False,    },    transition: {        duration: 750,        easing: 'cubic-in-out',    },}):
...
- config (dict; optional):
    ...
    https://plot.ly/javascript/configuration-options/ for more info.
...
- extendData (list | dict; optional):
    ...
    array containing data objects to extend, `traceIndices` (optional)
    is an array of trace indices that should be extended, and
    `maxPoints` (optional) is either an integer defining the maximum
    number of points allowed or an object with key:value pairs
    matching `updateData` Reference the Plotly.extendTraces API for
    full usage:
    https://plot.ly/javascript/plotlyjs-function-reference/#plotlyextendtraces.

- figure (dict; default {data: [], layout: {}, frames: []}):
    Plotly `figure` object. See schema:
    https://plot.ly/javascript/reference  `config` is set separately
...
- prependData (list | dict; optional):
    ...
    `[updateData, traceIndices, maxPoints]`, where `updateData` is an
    array containing data objects to prepend, `traceIndices`
    (optional) is an array of trace indices that should be extended,
    and `maxPoints` (optional) is either an integer defining the
...
- responsive (a value equal to: true, false, 'auto'; default 'auto'):
    ...
    `figure.layout.width`. This is the legacy behavior of the
    ExtendableGraph component.  Needs to be combined with appropriate
    dimension / styling through the `style` prop to fully take effect.
